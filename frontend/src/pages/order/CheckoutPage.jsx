...
export default function CheckoutPage() {
    ...
    const [stepStatus, setStepStatus] = useState({ // Tracks completion for visual timeline
        1: 'pending', // 'pending', 'completed', 'active'
        2: 'pending',
        3: 'pending',
    });
    ...
    const [shippingAddress, setShippingAddress] = useState({
        ...
        city: '', state: '', zipCode: '', country: 'India', phoneNumber: ''
    });
                                    ...
                                    {isRegistering ? (
                                        <form onSubmit={handleRegisterSubmit} className="space-y-4">
                                            <div>
                                                ...
                                                <input type="text" id="regName" name="name" value={authFormData.name} onChange={handleAuthFormChange} className="mt-1 block w-full p-2 border rounded-md" required />
                                            </div>
                                            <div>
                                                <label htmlFor="regEmail" className="block text-sm font-medium text-gray-700">Email</label>
                                                <input type="email" id="regEmail" name="email" value={authFormData.email} onChange={handleAuthFormChange} className="mt-1 block w-full p-2 border rounded-md" required />
                                            </div>
                                            <div>
                                                <label htmlFor="regPassword" className="block text-sm font-medium text-gray-700">Password</label>
                                                <input type="password" id="regPassword" name="password" value={authFormData.password} onChange={handleAuthFormChange} className="mt-1 block w-full p-2 border rounded-md" required />
                                            </div>
                                            <div>
                                        ...
                                        <form onSubmit={handleLoginSubmit} className="space-y-4">
                                            <div>
                                                ...
                                                <input type="email" id="loginEmail" name="email" value={loginFormData.email} onChange={handleAuthFormChange} className="mt-1 block w-full p-2 border rounded-md" required />
                                            </div>
                                            <div>
                            ...
                            <form onSubmit={handleShippingSubmit} className="space-y-6 pt-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" id="fullName" name="fullName" value={shippingAddress.fullName} onChange={handleShippingChange} className="mt-1 block w-full p-2 border rounded-md" required />
                                    </div>
                                    <div>
                                        <label htmlFor="phoneNumber" className="block text-sm font-medium text-gray-700">Phone Number</label>
                                        <input type="tel" id="phoneNumber" name="phoneNumber" value={shippingAddress.phoneNumber} onChange={handleShippingChange} className="mt-1 block w-full p-2 border rounded-md" required />
                                    </div>
                                </div>
                                <div>
                                    <label htmlFor="addressLine1" className="block text-sm font-medium text-gray-700">Address Line 1</label>
                                    <input type="text" id="addressLine1" name="addressLine1" value={shippingAddress.addressLine1} onChange={handleShippingChange} className="mt-1 block w-full p-2 border rounded-md" placeholder="House number, Street name" required />
                                </div>
                                <div>
                                    <label htmlFor="addressLine2" className="block text-sm font-medium text-gray-700">Address Line 2 (Optional)</label>
                                    <input type="text" id="addressLine2" name="addressLine2" value={shippingAddress.addressLine2} onChange={handleShippingChange} className="mt-1 block w-full p-2 border rounded-md" placeholder="Apartment, suite, unit, building, floor etc." />
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label htmlFor="city" className="block text-sm font-medium text-gray-700">City</label>
                                        <input type="text" id="city" name="city" value={shippingAddress.city} onChange={handleShippingChange} className="mt-1 block w-full p-2 border rounded-md" required />
                                    </div>
                                    <div>
                                        <label htmlFor="state" className="block text-sm font-medium text-gray-700">State / Province</label>
                                        <input type="text" id="state" name="state" value={shippingAddress.state} onChange={handleShippingChange} className="mt-1 block w-full p-2 border rounded-md" required />
                                    </div>
                                    <div>
                                        <label htmlFor="zipCode" className="block text-sm font-medium text-gray-700">Zip Code</label>
                                        <input type="text" id="zipCode" name="zipCode" value={shippingAddress.zipCode} onChange={handleShippingChange} className="mt-1 block w-full p-2 border rounded-md" required />
                                    </div>
                                </div>
                                <button
                                    type="submit"
                    ...
                    <div className={`bg-white shadow-md rounded-lg p-6 ${currentStep < 3 ? 'opacity-50 pointer-events-none' : ''}`}> {/* Visually disable if not active step */}
                        ...
                        <div className={`mt-2 overflow-hidden transition-all duration-500 ease-in-out ${openStep === 3 ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="py-4">
                                ...
                                <h3 className="text-xl font-semibold text-gray-700 mb-4">Shipping To:</h3>
                                {shippingAddress.fullName ? (
                                    <div className="text-sm text-gray-700 space-y-1 mb-4 border-b pb-4">
