...
// src/components/ShopPage.jsx (or src/pages/ShopPage.jsx)
// src/components/ShopPage.jsx (or src/pages/ShopPage.jsx)
import React, { useState, useEffect } from 'react';
import { FiFilter, FiX, FiHeart, FiStar, FiShoppingBag, FiChevronDown, FiChevronUp } from 'react-icons/fi';
import { FaGem, FaRing, } from 'react-icons/fa';
...
const ShopPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="fixed top-0 left-0 right-0 bg-white shadow-sm z-40 p-4 flex justify-between items-center">
      ...
      {/* Overlay for Filter Panel (now visible on all screen sizes) */}
      {isFilterPanelOpen && (
        <div
      ...
      <div className="flex flex-col lg:flex-row max-w-8xl mx-auto pt-16"> {/* Removed lg:pt-0, adjust padding */}
        {/* Filter Panel (Sidebar - always fixed and toggled) */}
        <aside
          className={`absolute top-0 left-0 h-full w-64 bg-white shadow-lg z-50 transform
        ...
        >
          <div className="flex justify-between items-center mb-6"> {/* Removed lg:hidden */}
            ...
            <button onClick={() => setIsFilterPanelOpen(false)} className="text-gray-700">
          ...
          <button
            onClick={() => setFilters({ metals: [], categories: [], priceRange: [0, 20000] })}
            className="bg-amber-700 text-white px-6 py-2 rounded-lg hover:bg-amber-800 w-full"
          >
            Reset Filters
          </button>
        </aside>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredProducts.map(product => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      <Link to={`/products/${product.slug}`} className="block">
                        <h3 className="text-lg font-semibold text-gray-800 truncate hover:text-blue-700 transition">
                          {product.title}
                        </h3>
                      </Link>
                      ...
                      <button className="mt-4 w-full bg-violet-800 text-white py-2 rounded-md hover:bg-violet-700 transition flex items-center justify-center space-x-2">
                        <FiShoppingBag />
      ...
      {/* Featured Categories Banner (kept static for now, but can be dynamic) */}
      <div className="max-w-7xl mx-auto px-4 py-12">
        <h3 className="text-2xl font-bold text-center text-gray-800 mb-8">Shop By Category</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {Object.entries(categoryIcons).map(([categoryName, icon]) => (
            ...
            >
              ...
              <Link to="/shop" state={{ categoryId: `some-${categoryName.toLowerCase()}-id`, categoryName: categoryName }} className="text-blue-500 hover:underline text-sm mt-2 block">
                Shop Now
              </Link>
            </div>
          ))}
